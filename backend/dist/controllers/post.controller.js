    ...
    if (url) {
        ...
        if (data) {
            const imageUrls = Array.isArray(data.result.ogImage)
                ? data.result.ogImage.map((img) => img.url).filter(Boolean)
                : [];
            linkPreview.push({
...
export const getUserPostController = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { posts: userPost, currentPage: page, totalPages: totalPages }, "User Posts fetched successfully!"));
});
...
export const searchPostsController = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { posts, currentPage, totalPages }, "Posts fetched successfully!"));
});
...
export const publishDraftController = asyncHandler(async (req, res) => {
    ...
    if (!draft) {
        throw new ApiError(404, "Draft not found or you don't have permission to publish this draft");
    }
